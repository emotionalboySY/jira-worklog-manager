# DK 워크로그 데스크톱 위젯 (JiraWorklogWidget)
...
5. 릴리스 생성 (소스 repo는 private — installer는 **별도 public repo**에 올림):
   ```sh
   gh release create vX.Y.Z --repo emotionalboySY/dk-worklog-widget-releases \
     --title "vX.Y.Z" --notes "..." <setup.exe> <latest.json>
   ```
6. updater 엔드포인트 확인: `https://github.com/emotionalboySY/dk-worklog-widget-releases/releases/latest/download/latest.json` 이 새 버전을 반환하는지
